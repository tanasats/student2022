...
export class ActorganizationService {

  private endpoint = 'http://localhost:3000/api/v1/actorganization';
  ...
  update(datas: any): Observable<any> {
    return this.http
      .put(this.endpoint + '/' + datas.actorganization_id, datas, this.httpOptions)
      .pipe(catchError(this.handleError));
  }
          ...
          .subscribe({
            next: (res: any) => {
              const datas = res.items.map((item: any) => {
                return {
                  value: item.actorganization_id,
                  name: item.actorganization_name,
                  selected: false,
                };
              });
              observer.next(datas);
